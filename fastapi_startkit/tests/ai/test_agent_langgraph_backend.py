...
def test_instructions_lead_the_message_list():
    messages = JobAssistant()._build_messages("find me a job")

    assert messages[0] == {"role": "system", "content": "You help users find jobs."}
...
def test_instructions_can_be_a_method_override():
    ...
    assert messages[0] == {"role": "system", "content": "Computed identity."}


def test_no_instructions_prepends_no_system_message():
