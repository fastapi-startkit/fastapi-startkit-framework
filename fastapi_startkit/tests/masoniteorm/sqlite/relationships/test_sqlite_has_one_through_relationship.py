...
class TestHasOneThroughRelationship(TestCase):
    ...
    async def test_has_one_through_has_query_without_callback(self):
        shipments = await IncomingShipment.where_has("from_country").get()
        filtered = await IncomingShipment.where_has("from_country", lambda query: query).get()
        assert shipments.count() == filtered.count() > 0

    async def test_has_one_through_relate(self):
        shipment = await IncomingShipment.where("name", "Milk").first()
        country = await IncomingShipment.from_country.relate(shipment).first()
        assert isinstance(country, Country)
        assert country.country_id == 10

    async def test_has_one_through_eager_load_with_callback(self):
        seen = []
