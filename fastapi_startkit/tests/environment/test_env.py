...
class EnvTest(unittest.TestCase):
    ...
    def test_numeric_string_cast_to_int(self):
        ...
        self.assertEqual(env("TEST_INT", 0), 6379)
        self.assertIsInstance(env("TEST_INT", 0), int)

    def test_zero_string_cast_to_int(self):
        os.environ["TEST_INT"] = "0"
        self.assertEqual(env("TEST_INT", 1), 0)
        self.assertIsInstance(env("TEST_INT", 1), int)

    # ── boolean auto-cast ─────────────────────────────────────────────────────

    def test_true_lowercase_cast_to_bool(self):
        os.environ["TEST_BOOL"] = "true"
        self.assertIs(env("TEST_BOOL", False), True)
    ...
    def test_false_lowercase_cast_to_bool(self):
        ...
        self.assertIs(env("TEST_BOOL", True), False)
    ...
    def test_default_is_required(self):
        with self.assertRaises(TypeError):
            env("TEST_STR")  # type: ignore[call-arg]
    ...
    def test_cast_false_keeps_numeric_as_str(self):
        os.environ["TEST_CAST"] = "6379"
        self.assertEqual(env("TEST_CAST", None, cast=False), "6379")
        self.assertIsInstance(env("TEST_CAST", None, cast=False), str)

    def test_cast_false_keeps_true_as_str(self):
        os.environ["TEST_CAST"] = "true"
        self.assertEqual(env("TEST_CAST", None, cast=False), "true")
        self.assertIsInstance(env("TEST_CAST", None, cast=False), str)

    def test_cast_false_keeps_false_as_str(self):
        os.environ["TEST_CAST"] = "false"
        self.assertEqual(env("TEST_CAST", None, cast=False), "false")
        self.assertIsInstance(env("TEST_CAST", None, cast=False), str)
