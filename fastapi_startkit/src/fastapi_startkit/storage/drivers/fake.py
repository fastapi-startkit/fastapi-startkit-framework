...
class FakeDriver(LocalDriver):
    ...
    def get_files(self, directory=""):
        """List files directly under ``directory`` in the fake's temp root.

        Delegates to LocalDriver rather than reimplementing the listing so the
        fake can never drift from the semantics it stands in for.
        """
